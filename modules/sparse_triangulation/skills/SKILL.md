...
| If | Fetch |
| --- | --- |
| a number is out of band, or a diagnostic told you to tune | **`tuning`** — 4 parameters documented, starting with `min_triangulation_angle_deg` |
| you need to know whether this module can do the thing at all | **`limitations`** — e.g. "Cheirality failures are a pose problem" |
...
**Diagnostics it can raise:** `no_points`, `bad_poses_suspected`, `two_view_cloud`, `weak_geometry`.
