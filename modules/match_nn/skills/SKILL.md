...
| If | Fetch |
| --- | --- |
| a number is out of band, or a diagnostic told you to tune | **`tuning`** — 8 parameters documented, starting with `pairing` |
| you need to know whether this module can do the thing at all | **`limitations`** — e.g. "Finding a replacement" |
...
**Diagnostics it can raise:** `broken_chain`, `low_inlier_ratio`, `sparse_matches`, `degenerate_geometry`.
