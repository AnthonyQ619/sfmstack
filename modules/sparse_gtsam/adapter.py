...
def run(ctx: Ctx):
    ...
    # The PER-POINT mean, not the per-observation mean. `residuals` is one row
    # per observation, so averaging it directly weights long tracks by their
    # length -- and long tracks are the higher-error points, so the published
    # figure came out inflated against every other producer of this type.
    # SparseTriangulation, SparseGlobalCOLMAP and pycolmap all publish the point
    # mean; this module was the only one that did not, and the gap was large
    # enough to invert the head-to-head comparison its own SKILL prints.
