# Driving one capture to a refined sparse model

You are driving a structure-from-motion pipeline for ONE capture, from raw frames
to a refined sparse model. You have never seen this capture. Everything you know
...
4. **Stop at the refined sparse model.** Dense reconstruction is out of scope.
...
A step can take many minutes on a shared machine. Give each `sfmx run` call a Bash
timeout of 600000 ms. If a call times out, the step is still running: call
`sfmx wait` until it reports nothing in flight, then read the step from
`sfmx state`. Do not re-issue a run that is still in flight. One capture runs one
step at a time; a second `run` waits for the first.
...
8. **Optimization.** The refined sparse model this produces is the deliverable.
...
- Budget roughly 25-40 runs. Stop earlier when the model is clearly good and further
  moves are not paying.
...
{
  "capture": "<capture>",
  "final": {"refined_artifact": "", "scene_artifact": "",
...
`refined_artifact` is the optimization-stage model you are delivering. If the
service kept a second solve for it, deliver the model the service kept and say so.
