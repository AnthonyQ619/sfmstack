# Worked Runs — Raw Evidence

**This file is a citation record, not a retrieval aid.** Every table below is the
raw per-capture measurement behind a claim stated elsewhere as a scene *property*.
Scene names are kept deliberately, and only here, so a claim can be traced back and
re-run.

**Do not plan from a row.** A plan for a new capture cannot use a row from these
tables — only the reasoning built on them, which lives in
[`skills/scene_to_pipeline.md`](../scene_to_pipeline.md),
[`skills/families/`](../families/) and [`skills/judgment/`](../judgment/). Matching
your own readings against a row here to find "the capture like mine" is the failure
mode this file is most likely to cause: a reading that agrees to several digits
usually means you are reading your own capture back.

For "has a scene like this been solved before?" — the trait-keyed retrieval table —
see [INDEX.md](INDEX.md). The two are separated because they are incompatible:
retrieval wants a row you can match against, and this file exists to be cited and
not matched.
...
`families/`, `judgment/` and the module skills in the current round. **Its raw
per-capture record is not in this file.** The captures it ran on are listed in
[CORPUS.txt](CORPUS.txt), which is what pins the scope of every range in
`scene_to_pipeline.md`, but the per-run numbers are not yet citable from here.

That is a known gap rather than an oversight: it is the first body of evidence
large enough that transcribing it by hand is the wrong move, and it is waiting on a
decision about whether the distillation loop writes this file. Until then, a claim
attributed to "a seventeen-capture sweep" is traceable to its scope and not to its
rows.
