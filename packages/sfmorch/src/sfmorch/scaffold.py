...
import yaml
from sfmkit import registry as core_types
...
def _manifest(r: ScaffoldRequest, slug: str) -> str:
    ...
    # Required first, generated from the produced types, then a slot for whatever
    # this module measures that its siblings do not. The required ones are not
    # suggestions -- the registry refuses a manifest without them, and seal()
    # refuses an artifact that declares and never emits them.
    metrics: dict[str, Any] = {}
    for type_name in r.produces.values():
        if type_name not in core_types():
            continue
        for name, required in core_types().get(type_name).metrics.items():
            metrics.setdefault(name, {
                "direction": required.direction,
                "healthy": [None, None],
                "meaning": required.meaning.strip(),
            })
    ...
    header = dedent(f"""\
        ...
        #
        # The metrics carried over from the produced type(s) are REQUIRED. Set their
        # `healthy` bands -- the band is method-specific and only you know it -- but
        # do not remove them: they are what makes this module interchangeable with
        # the others at its stage.
        """)
